...
export default function ViewSchedule() {
  ...
  }, [token, fetchSchedules, fetchAppointments]);
  ...
  const selectedAppointments = useMemo(
    () => appointments.filter((appointment) => appointment.appointmentDate?.startsWith(selectedDate)),
  ...
  const handleCreateSchedule = async (e) => {
    ...
    if (selectedDate < today) {
      setError("Cannot create schedule for a past date.");
      return;
    }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setDeleteLoading(id);
      await deleteMySchedule(id, token);
      await fetchSchedules();
    } catch (e) {
  ...
  const handleStop = async (id) => {
    ...
    try {
      setStopLoading(id);
      await stopMySchedule(id, token);
      await fetchSchedules();
    } catch (e) {
      alert(e.message || "Failed to stop schedule.");
    } finally {
      setStopLoading(null);
    }
  };
  ...
  const handleResume = async (id) => {
    try {
      setStopLoading(id);
      await resumeMySchedule(id, token);
      await fetchSchedules();
    } catch (e) {
      alert(e.message || "Failed to resume schedule.");
    } finally {
      setStopLoading(null);
    }
  };
  ...
  const goPrev = () => {
    ...
    const nextDate = fromDateKey(selectedDate);
  ...
  const selectedDayLabel = formatHumanDate(selectedDate);

  return (
    <DashboardLayout title="My Schedule" links={DOCTOR_LINKS}>
      <div className="space-y-6">
        <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <h2 className="text-xl font-bold text-gray-900">Google Calendar Style Schedule</h2>
          <p className="mt-1 text-sm text-gray-500">
            Create time slots by selecting a day on the calendar and entering the slot times below.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              <h3 className="text-base font-bold text-gray-900">{selectedDayLabel}</h3>
              ...
              {selectedSchedule ? (
                ...
                <div className="mt-4 rounded-xl bg-gray-50 p-3 text-sm text-gray-600">
                  Select a future day to create a slot.
                </div>
              ) : (
                <form onSubmit={handleCreateSchedule} className="mt-4 space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      required
                      className="w-full rounded-lg border border-gray-200 bg-gray-50/50 px-3 py-2 text-sm outline-none focus:border-primary-500 focus:bg-white focus:ring-4 focus:ring-primary-500/10"
                    />
                  </div>
              ...
              <div className="mt-5 border-t pt-4">
                <h4 className="text-sm font-semibold text-gray-900">Selected day appointments</h4>
                <div className="mt-3 space-y-2">
                  {selectedAppointments.length === 0 ? (
                    <p className="text-sm text-gray-500">No appointments on this day.</p>
                  ) : (
                    selectedAppointments.slice(0, 5).map((appointment) => (
                      <div key={appointment.id} className="rounded-lg bg-gray-50 px-3 py-2 text-sm">
                        <div className="font-medium text-gray-900">{appointment.patientName || "Patient"}</div>
                        <div className="text-gray-500">{appointment.status || "Scheduled"}</div>
                      </div>
                    ))
                  )}
                </div>
