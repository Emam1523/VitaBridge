...
import { getAssistantProfile, updateAssistantProfile } from "../../api/assistantApi";
...
const inputCls = (disabled) =>
  `mt-1.5 block w-full rounded-xl border px-4 py-2.5 text-sm transition outline-none ${
    disabled
      ? "border-gray-100 bg-gray-50 text-gray-500 cursor-default"
      : "border-gray-200 bg-white text-gray-900 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10"
  }`;
...
export default function AssistantProfile() {
  ...
  const [form, setForm] = useState({
    name: "", phoneNumber: "", gender: "", dateOfBirth: "", address: "", bio: "", employeeId: "",
  });
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const data = await getAssistantProfile(token);
        setProfile(data);
        populate(data);
      } catch (err) {
  ...
  const submit = async (e) => {
    ...
    try {
      setSaving(true);
      const updated = await updateAssistantProfile(form, token);
      setProfile(updated);
      populate(updated);
  ...
  if (loading && !profile) {
    return (
      <DashboardLayout title="My Profile" links={ASSISTANT_LINKS}>
            ...
            <div>
              ...
              {profile?.assignedDoctorName && (
                <p className="mt-0.5 text-xs text-primary-600 font-medium">
                  Assigned to Dr. {profile.assignedDoctorName}
                </p>
              )}
            </div>
          </div>
            ...
            {activeTab === "personal" && (
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <input type="text" name="employeeId" value={form.employeeId} onChange={change} disabled={!isEditing} placeholder="e.g., EMP-001" className={inputCls(!isEditing)} />
                </div>
                <div>
                ...
                {profile?.assignedDoctorName && (
                  <div className="sm:col-span-2">
                    <label className="text-sm font-medium text-gray-700">Assigned Doctor</label>
                    <input type="text" value={`Dr. ${profile.assignedDoctorName}`} disabled className={inputCls(true)} />
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === "about" && (
              <div>
                ...
                <textarea
                  ...
                  onChange={change}
                  disabled={!isEditing}
                  placeholder="Tell us about yourself, your experience, skills, and responsibilities..."
                  className={`${inputCls(!isEditing)} resize-none`}
                />
                {!isEditing && !form.bio && (
                  <p className="mt-3 rounded-xl bg-gray-50 p-4 text-center text-sm text-gray-400">
